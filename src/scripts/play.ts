...
function buildOptions(rl: Interface, game: Game, playerId: PlayerId): MenuOption[] {
  ...
  const freeControlled = game.board
    ...
    .map((n) => n.id);
  const scoutCells = handTypes.includes("explorador")
    ? [...new Set(ownUnits(game, playerId).flatMap((u) => game.board.getNeighbors(u.position).filter((p) => game.board.unitAt(p) === undefined)))]
    : [];
  // Solo tipos con AL MENOS una casilla válida (desplegables y con sitio): el
  // Explorador vale con controladas O adyacentes a un aliado; el resto solo
  // con controladas libres. Así el pickFromList de debajo nunca recibe una
  // lista vacía y no se muestra el menú si el filtrado deja vacío.
  const deployableWithCells = handTypes.filter((t) =>
    unitsOf(game, playerId, t).length < (t === "infanteria" ? 2 : 1)
    && (t === "explorador" ? freeControlled.length > 0 || scoutCells.length > 0 : freeControlled.length > 0),
  );
  if (deployableWithCells.length > 0) {
    options.push({
      label: `Desplegar (${deployableWithCells.map((t) => UNIT_NAMES[t]).join(", ")})`,
      run: async () => {
        const type = await pickFromList(rl, "Tipo a desplegar", deployableWithCells, (t) => `${UNIT_NAMES[t]} (${UNIT_TOTAL_COINS[t]} monedas)`);
